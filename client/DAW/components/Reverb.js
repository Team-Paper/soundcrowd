...
const ReverbModal = function ({ projectId, track, setTrackReverbGain, toggleTrackReverb, setTrackReverb, reverbs }) {
  const reverbOptions = Object.entries(reverbs).map(([key, reverb]) => ({ key, value: key, text: reverb.title }));
  ...
  return (
    <Modal trigger={<Button color={track.reverb.on ? 'grey' : ''} size="mini">R</Button>}>
      ...
      <Modal.Content>
        <Modal.Description>
        On: <Checkbox toggle checked={!!track.reverb.on} onChange={() => toggleTrackReverb(projectId, track)} />
          <label>Gain: </label><input type="range" value={track.reverb.gain} onChange={e => setTrackReverbGain(projectId, track, e.target.value)} max={1} min={0} step={0.01} />
